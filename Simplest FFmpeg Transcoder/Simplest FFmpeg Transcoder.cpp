// Simplest FFmpeg Transcoder.cpp : 定义控制台应用程序的入口点。
//
...
static int open_input_file(const char *filename)
{
	...
	if (ret < 0)
	{
		av_log(NULL, AV_LOG_ERROR, "Can't openinput file.\n");
		return ret;
	}
...
static int open_output_file(const char *filename)
{
	...
	for (i = 0; i < ifmt_ctx->nb_streams; i++)
	{
			...
			|| dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO)
		{
			...
			* streams easily using filters */
			...
			if (ret < 0)
			{
				av_log(NULL, AV_LOG_ERROR, "Can't openvideo encoder for stream #%u.\n", i);
				return ret;
			}
		}
	...
	if (!(ofmt_ctx->oformat->flags &AVFMT_NOFILE))
	{
		...
		if (ret < 0)
		{
			av_log(NULL, AV_LOG_ERROR, "Can't open output file '%s'.", filename);
			return ret;
		}
	}
	...
	AVCodecContext *enc_ctx, const char *filter_spec)
{
	...
	else if (dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO)
	{
		...
		if (ret < 0)
		{
			av_log(NULL, AV_LOG_ERROR, "Can't create audio buffer source.\n");
			goto end;
		}
		ret = avfilter_graph_create_filter(&buffersink_ctx, buffersink, "out",
			NULL, NULL, filter_graph);
		if (ret < 0)
		{
			av_log(NULL, AV_LOG_ERROR, "Can't create audio buffer sink.\n");
			goto end;
		}
		...
		if (ret < 0)
		{
			av_log(NULL, AV_LOG_ERROR, "Can't set output sample format.\n");
			goto end;
		}
		...
		if (ret < 0)
		{
			av_log(NULL, AV_LOG_ERROR, "Can't set output channel layout.\n");
			goto end;
		}
		...
		if (ret < 0)
		{
			av_log(NULL, AV_LOG_ERROR, "Can't set output sample rate.\n");
			goto end;
		}
